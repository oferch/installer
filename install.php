...
function installationFailed($error) {
	...
	logMessage(LOG_USER, "An error has occured during installation: $error");
	logMessage(LOG_USER, "Cleaning leftovers...");	
	detectLeftovers(false);
//	if ($shouldReport) {
//		reportInstallationFailure();
//	}	
	logMessage(LOG_USER, $texts["flow"]["install_fail"]);
	die(1);
}
...
function detectLeftovers($report_only) {
	...
	if (isset($leftovers)) {
		if ($report_only) logMessage(LOG_USER, "Installation leftovers are: $leftovers");
		return true;
	} else {
		return false;
	}	
...
$installation_start = date("d.m.Y_H:i:s");
startLog($installation_start);
logMessage(LOG_INFO, "Installation started");
$texts = parse_ini_file('installer/texts.ini');

// installation might take a few minutes
ini_set('max_execution_time', 0);
ini_set('memory_limit', -1);
ini_set('max_input_time ', 0);
logMessage(LOG_INFO, "Installation might take a few minutes, set PHP ini values: max_execution_time=0; memory_limit=-1;max_input_time=0");
...
try {
	...
	logMessage(LOG_INFO, "Installing ".'Kaltura '.$version['type'].' '.$version['number']);
}
catch (Exception $e) {
	installationFailed($texts["flow"]['package_corrupt']);
}
...
logMessage(LOG_USER, $texts["flow"]['welcome_msg']);
...
if ($result = ((strcasecmp($app_config['KALTURA_VERSION_TYPE'], 'TM') == 0) || 
	...
	$email = UserInputUtils::getInput('REPORT_MAIL', $texts["flow"]['report_mail']);	
...
if ($should_user_input) logMessage(LOG_USER, $texts["flow"]['config_start']);
else logMessage(LOG_USER, $texts["flow"]['skipping_input']);
...
if ($should_user_input) logMessage(LOG_USER, $texts["input"]['admin_console_welcome']);
...
logMessage(LOG_USER, $texts["flow"]['prereq_start']);
...
if (detectLeftovers(true)) {
	...
	logMessage(LOG_INFO, "No previous installation leftovers were found, installation can proceed");
}

// installation

logMessage(LOG_USER, $texts["flow"]["starting_installation"]);
...
logMessage(LOG_USER, $texts["flow"]["replacing_tokens"]);
...
logMessage(LOG_USER, sprintf($texts["flow"]["adjusting_architecture"], $os_name, $architecture));
...
// chmod
logMessage(LOG_USER, $texts["flow"]["chmoding"]);
...
logMessage(LOG_INFO, "Setting-up Kaltura stats DB");
...
logMessage(LOG_USER, $texts["flow"]["dwh"]);
...
logMessage(LOG_USER, $texts["flow"]["symlinks"]);
foreach ($installation_config['symlinks']['links'] as $slink) {
	...
	else logMessage(LOG_INFO, "Created symblic link from $link_items[0] to $link_items[1]");
}

logMessage(LOG_USER, $texts["flow"]["config_system"]);
...
// post install
logMessage(LOG_USER, $texts["flow"]["uninstaller"]);
...
logMessage(LOG_USER, $texts["flow"]["run_system"]);
@exec($app_config['APP_DIR'].'/scripts/serviceBatchMgr.sh start  2>&1');
@exec($app_config['APP_DIR'].'/scripts/searchd.sh start  2>&1');
//sendSuccessMail();
logMessage(LOG_USER, sprintf($texts["flow"]["install_success"], $app_config['ADMIN_CONSOLE_ADMIN_MAIL'], $app_config['ADMIN_CONSOLE_PASSWORD']));
logMessage(LOG_USER, sprintf($texts["flow"]["after_install_steps"], $app_config["KALTURA_VIRTUAL_HOST_NAME"], $app_config["KALTURA_VIRTUAL_HOST_NAME"], $app_config["BASE_DIR"], $app_config["HTTPD_BIN"], $app_config["KALTURA_VIRTUAL_HOST_NAME"]));
